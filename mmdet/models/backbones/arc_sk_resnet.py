...
class ARCSKResNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 replace=None,
                 kernel_number=None):
        super(ARCSKResNet, self).__init__()
    ...
    def train(self, mode=True):
        ...
        freezed"""
        super(ARCSKResNet, self).train(mode)
        self._freeze_stages()
...
def batch_rotate_multiweight(weights, lambdas, thetas):
    ...
    # _rotation_matrix = rotation_matrix.view(b * 9, n, 9)
    ...
    # _weights = weights.view(n, 9, Cout * Cin)
    ...
    temp_weight_list = []
    for i in range(n):
        _temp_weight = torch.mm(rotation_matrix[:, i * 9:(i+1) * 9], weights[i * 9: (i+1) * 9 :])
        temp_weight_list.append(_temp_weight)
    weights = torch.stack(temp_weight_list, dim=1)
    # temp_weight_1 = torch.einsum('bij,ijk->bik', _rotation_matrix, _weights)
    # weight = torch.mm(rotation_matrix, weights)
    ...
    weights = weights.contiguous().view(b, 3, 3, 4, Cout, Cin)
    weights = weights.permute(0, 3, 4, 5, 1, 2)
    weights = weights.reshape(b * n * Cout, Cin, 3, 3)
...
class AdaptiveRotatedConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        d = max(in_channels // 16, 32)
        self.fc1 = nn.Sequential(nn.Conv2d(out_channels, d, 1, bias=False),
                                 nn.BatchNorm2d(d),
                                 nn.ReLU(inplace=True))  # 降维

        self.fc2 = nn.Conv2d(d, out_channels * self.kernel_number, 1, 1, bias=False)  # 升维
    ...
    def forward(self, x):
        ...
        # rotate weight
        # # [Cout, Cin, k, k] --> [bs * kernel_num * Cout, Cin, k, k]
        rotated_weight = self.rotate_func(self.weight, alphas, angles)
        # reshape images
        bs, Cin, h, w = x.shape
        x = x.repeat(1, self.kernel_number, 1, 1)
        x = x.reshape(1, bs * self.kernel_number * Cin, h, w)  # [1, bs * Cin, h, w]
        ...
        out = F.conv2d(input=x, weight=rotated_weight, bias=None, stride=self.stride, padding=self.padding,
                       dilation=self.dilation, groups=(self.groups * bs * self.kernel_number))
        out = out.reshape(bs, self.kernel_number, Cin, *out.shape[-2:])

        weight_vector = self.global_pool(out.sum(dim=1))
        weight_vector = self.fc2(self.fc1(weight_vector))
        weight_vector = self.softmax(weight_vector.reshape(bs, self.kernel_number, self.out_channels, -1))
        out = (weight_vector[..., None] * out).sum(dim=1)
        # reshape back
